    ...
    async function loadOrderStatus() {
      try {
        ...
        if (cancelled || !response.ok || !data?.order) {
          setCheckoutNotice(
            'Платёж отправлен в обработку. Как только оплата подтвердится, доступ появится в кабинете.'
          );
          return;
        }
        ...
        <div className="lg:col-span-1">
          <Card hover={false} className="sticky top-20">
            ...
            <p className="mt-3 text-xs text-gray-500 leading-relaxed">
              Сумма заказа и скидки подтверждаются на сервере прямо перед оплатой. После оплаты
